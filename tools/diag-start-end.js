...
(async () => {
  ...
  const d = await page.evaluate(() => {
    const p = document.getElementById('wlPlan');
    const planDays = window.__planDays;
    return {
      planDisp: p ? p.style.display : 'NO_ELEMENT',
      planHtml: p ? p.innerHTML.slice(0, 300) : 'NO_ELEMENT',
      hasPlanStart: !!document.getElementById('planStart'),
      hasPlanEnd: !!document.getElementById('planEnd'),
      hasPlanLoop: !!document.getElementById('planLoop'),
      __planDays: Array.isArray(planDays) ? planDays.length : 'none'
    };
  });
