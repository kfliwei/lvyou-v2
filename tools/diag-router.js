...
const demoB = '<!DOCTYPE html><html><head><meta charset="utf-8"></head><body><div id="app"><h1 id="pageTitle">页面B</h1></div><script>window.__demoB=1;</script></body></html>';
...
(async () => {
  ...
  const r = await page.evaluate(async () => {
    const out = {};
    out.before = document.getElementById('pageTitle').textContent;
    try {
      const res = await fetch('b.html');
      out.fetchStatus = res.status;
      const html = await res.text();
      out.htmlLen = html.length;
      const doc = new DOMParser().parseFromString(html, 'text/html');
      out.parsedTitle = doc.getElementById('pageTitle').textContent;
      out.bodyLen = doc.body.innerHTML.length;
